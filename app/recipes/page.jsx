  ...
  return (
    <div className="min-h-screen bg-white text-neutral-950">
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-20 md:px-6 lg:px-8">
        <section className="rounded-md border-2 border-[#d5e8d7] bg-white px-5 py-8 sm:px-8 sm:py-10">
        ...
        <section className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {recipes.map((recipe) => (
            <article key={recipe.slug} className="rounded-md border-2 border-[#d5e8d7] bg-[#f8fbf8] p-5">
              <div className="relative mb-4 h-44 w-full overflow-hidden rounded-sm border border-neutral-300 bg-white">
                <Image
                  src={recipe.image}
                  alt={recipe.name}
                  fill
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-xs uppercase tracking-widest text-neutral-700">
                <div className="rounded-sm border border-neutral-300 bg-white px-2 py-2">
              ...
              <Link
                ...
                className="mt-5 inline-flex items-center rounded-sm border-2 border-[#4c8c51] bg-[#4c8c51] px-4 py-2 text-xs uppercase tracking-[0.14em] text-white transition hover:bg-white hover:text-[#4c8c51]"
              >
                View Recipe
              </Link>
            </article>
